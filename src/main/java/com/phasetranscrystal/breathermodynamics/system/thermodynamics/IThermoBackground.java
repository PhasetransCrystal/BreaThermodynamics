...
public interface IThermoBackground {
    ...
    default double interactWith(@Nullable IThermoBackground itbg, boolean simulate, BlockState target, Direction direction, BlockPos targetPos, @Nullable Level level) {
        if (itbg == null) {//如果目标为空气或其它无热力特性目标
            ...
            double q = thermalCycle(getK(), itbg.getK(), interactQuad(itbg, true, target, direction, targetPos, level), equT(itbg.getQ(), itbg.getMC()));
    ...
    default double insertHeat(double count, boolean simulate) {
        double cost = forceLimit() ? Math.min(count, maxQ() - getQ()) : count;
        if (!simulate) setQ(getQ() + cost);
    ...
    default int equT(double consumerQ, long consumerMC) {
